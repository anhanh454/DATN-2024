...
import com.example.ogani.utils.PayOsUtil;
import com.fasterxml.jackson.databind.JsonNode;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.lib.payos.Constants;
import com.lib.payos.PayOSError;
import com.lib.payos.Utils;
...
public class PayOs {
    ...
    public JsonNode createPaymentLink(PaymentData paymentData) throws Exception {
        ...
        paymentData.setSignature(PayOsUtil.createSignatureOfPaymentRequest(paymentData, this.checksumKey));
        System.out.println("Đây là signature: " + paymentData.getSignature());
        ...
        System.out.println("Đây là paymentDataJson: " + paymentDataJson);
        ...
        if (entity != null) {
            ...
            // In ra dữ liệu
            System.out.println("Đây là dữ liệu của entity: " + result.toString());
        }
        ...
        if (entity == null) {
            ...
            System.out.println("Đây là responseData: " + responseData.toString());
            response.close();
            client.close();
            JsonNode res = objectMapper.readTree(responseData);
            System.out.println("Đây là res: " + res.toPrettyString());
            if (!Objects.equals(res.get("code").asText(), "00")) {
                throw new PayOSError(res.get("code").asText(), res.get("desc").asText());
            } else {
                String paymentLinkResSignature = PayOsUtil.createSignatureFromObj(res.get("data"), this.checksumKey);
    ...
    public String confirmWebhook(String webhookUrl) throws Exception {
        if (webhookUrl != null && !webhookUrl.isEmpty()) {
            ...
            if (statusCode == 200) {
                ...
                throw new PayOSError((String) Constants.ERROR_CODE.get("INTERNAL_SERVER_ERROR"),
                        (String) Constants.ERROR_MESSAGE.get("INTERNAL_SERVER_ERROR"));
            } else if (statusCode == 401) {
                throw new PayOSError((String) Constants.ERROR_CODE.get("UNAUTHORIZED"),
                        (String) Constants.ERROR_MESSAGE.get("UNAUTHORIZED"));
            } else {
                throw new PayOSError((String) Constants.ERROR_CODE.get("INTERNAL_SERVER_ERROR"),
                        (String) Constants.ERROR_MESSAGE.get("INTERNAL_SERVER_ERROR"));
            }
        ...
        if (statusCode != 200) {
            ...
            if (entity == null) {
                ...
                if (!Objects.equals(res.get("code").asText(), "00")) {
                    ...
                    String paymentLinkInfoResSignature = Utils.createSignatureFromObj(res.get("data"),
                            this.checksumKey);
